...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function isValidEmail(email) {
    // Regex يرفض أي دومين غير رسمي زي .vm ويقبل .com, .net, .edu.eg إلخ
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", function (e) {
      e.preventDefault();
      const emailInput = document.getElementById("liEmail").value.trim();
      ...
      if (!user) return alert("This email is not registered. Please sign up first.");
      if (user.password !== passwordInput) return alert("Incorrect password!");

      // التحقق من صلاحية الأدمن
      if (isAdminLogin && user.role !== "admin") {
          return alert("Access denied. You do not have admin privileges!");
      }
